...
export const repo: IStaticDataRepo = {
  ...
  getCities: async () => {
    ...
    return data.map((row: any) => ({
      ...
      country_code: row.state?.country_id?.toString() ?? '',
      is_imp: row.is_imp ?? 0,
  ...
  getEducationDegrees: async () => {
    ...
    return data.map((row: any) => ({
      ...
      education_type_new: row.education_type ?? '',
      SORTBY: row.sortby ?? 0,
    }));
  },
    ...
    return data
      ...
      .map((row: any) => ({
        ...
        dependentSubCastes: data
          ...
          .flatMap((c: any) =>
            data
              .filter((s: any) => s.parent_id === c.id && s.level === 'Subcaste')
